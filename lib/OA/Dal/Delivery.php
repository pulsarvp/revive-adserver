...
//    $query = OA_Dal_Delivery_buildQuery('', '', '');
	...
	/**
	 * This function gets a campaign id by campaign name
	 *
	 * @param string $campaignname The campaign name
	 *
	 * @return int|false  campaign id
	 */
	function OA_Dal_Delivery_getCampaignIdByName ($campaignname)
	{
		$conf = $GLOBALS[ '_MAX' ][ 'CONF' ];
		...
		$query = "SELECT "
			. "campaignid AS campaign_id FROM " .
			OX_escapeIdentifier($conf[ 'table' ][ 'prefix' ] . $conf[ 'table' ][ 'campaigns' ])
			. " WHERE campaignname='{$campaignname}'";

		$rCampaigns = OA_Dal_Delivery_query($query);

		if (!OA_Dal_Delivery_isValidResult($rCampaigns))
		{
			return ( defined('OA_DELIVERY_CACHE_FUNCTION_ERROR') ) ? OA_DELIVERY_CACHE_FUNCTION_ERROR : false;
		}
		while ($aCampaign = OA_Dal_Delivery_fetchAssoc($rCampaigns))
		{
			return $aCampaign[ 'campaign_id' ];
		}
	...
	/**
	 * This function gets a zone id by zone name
	 *
	 * @param string $zonename The zone name
	 *
	 * @return int|false  zone id
	 */
	function OA_Dal_Delivery_getZoneIdByName ($zonename)
	{
		$conf = $GLOBALS[ '_MAX' ][ 'CONF' ];
		...
		$query = "SELECT"
			. " zoneid AS zone_id FROM " .
			OX_escapeIdentifier($conf[ 'table' ][ 'prefix' ] . $conf[ 'table' ][ 'zones' ])
			. " WHERE zonename='{$zonename}'";

		$rZones = OA_Dal_Delivery_query($query);
	...
	/**
	 * The function to get and return the ads linked to a zone and campaign
	 *
	 * @param  int   $zoneid     The id of the zone to get linked ads for
	 * @param  int   $campaignid The id of the campaign to get linked ads for
	 * @param  array $keywords   The keywords of the banner
	 *
	 * @todo   Refactor this query (and others) to use OA_Dal_Delivery_buildQuery()
	 * @return array|false
	 *               The array containg zone information with nested arrays of linked ads
	 ...
	 *                    creative weights
	 */
	function OA_Dal_Delivery_getZoneCampaignLinkedAdInfos ($zoneid, $campaignid, $keywords)
	{
		$conf = $GLOBALS[ '_MAX' ][ 'CONF' ];
		$zoneid = (int) $zoneid;
