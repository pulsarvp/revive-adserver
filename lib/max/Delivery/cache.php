<?php
	...
	*/
	...
	/**
	 * Cache-wrapper for OA_Dal_Delivery_getZoneCampaignLinkedAdInfos()
	 *
	 * The function to get and return the ads linked to a zone
	 *
	 * @param  int    $zoneid     The id of the zone to get linked ads for
	 * @param  int    $campaignid The id of the campaign to get linked ads for
	 * @param  array  $keywords   The keywords of the banner to get linked ads for
	 * @param boolean $cached     Should a cache lookup be performed?
	 *
	 * @return array|false
	 *                   The array containg zone information with nested arrays of linked ads
	 ...
	 *                        weight
	 */
	function MAX_cacheGetZoneCampaignLinkedAdInfos ($zoneId, $campagnid, $keywords, $cached = true)
	{
		$sName = OA_Delivery_Cache_getName(__FUNCTION__, $zoneId, $campagnid, $keywords);
		if (!$cached || ( $aRows = OA_Delivery_Cache_fetch($sName) ) === false)
		{
			MAX_Dal_Delivery_Include();
			$aRows = OA_Dal_Delivery_getZoneCampaignLinkedAdInfos($zoneId, $campagnid, $keywords);
			$aRows = OA_Delivery_Cache_store_return($sName, $aRows);
		}
	...
	/**
	 * Cache-wrapper for OA_Dal_Delivery_getCampaignIdByName()
	 *
	 * The function to get and return the ads linked to a zone
	 *
	 * @param  string $campaign The id of the campaign to get linked ads for
	 * @param boolean $cached   Should a cache lookup be performed?
	 *
	 * @return array|false
	 *
	 */
	function MAX_cacheGetCampaignIdByName ($campagn, $cached = true)
	{
		$sName = OA_Delivery_Cache_getName(__FUNCTION__, $campagn);
		if (!$cached || ( $aRows = OA_Delivery_Cache_fetch($sName) ) === false)
		{
			MAX_Dal_Delivery_Include();
			$aRows = OA_Dal_Delivery_getCampaignIdByName($campagn);
			$aRows = OA_Delivery_Cache_store_return($sName, $aRows);
		}
	...
	/**
	 * Cache-wrapper for OA_Dal_Delivery_getZoneIdByName()
	 *
	 * The function to get and return the ads linked to a zone
	 *
	 * @param  string $campaign The id of the campaign to get linked ads for
	 ...
	 *
	 */
	function MAX_cacheGetZoneIdByName ($zone, $cached = true)
	{
		$sName = OA_Delivery_Cache_getName(__FUNCTION__, $zone);
		if (!$cached || ( $aRows = OA_Delivery_Cache_fetch($sName) ) === false)
		{
			MAX_Dal_Delivery_Include();
			$aRows = OA_Dal_Delivery_getZoneIdByName($zone);
			$aRows = OA_Delivery_Cache_store_return($sName, $aRows);
		}
